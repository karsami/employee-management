...
export class UnauthenticatedContentComponent {
  ...
  get title() {
    ...
    switch (path) {
      ...
      case 'reset-password': return 'Khôi phục mật khẩu';
  ...
  get description() {
    const path = this.router.url.split('/')[1];
    switch (path) {
      case 'reset-password': return 'Hãy nhập email bạn đã dùng để đăng ký tài khoản, chúng tôi sẽ gửi link khôi phục mật khẩu vào địa chỉ email của bạn.';
      default: return '';
    }
  }
}
